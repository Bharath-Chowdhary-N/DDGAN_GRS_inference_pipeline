...
class NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.param_transform = nn.Sequential(
        nn.Linear(1, self.param_emb_dim),
        self.act,
        nn.Linear(self.param_emb_dim, self.param_emb_dim)
    )
  ...
  def forward(self, x, time_cond, z, param=None):
    ...
    if param is not None:
        # Make sure param is properly shaped and on the right device
        if isinstance(param, (float, int)):
            param = torch.tensor([param], device=x.device).view(1, 1).repeat(x.shape[0], 1)
        elif param.dim() == 1:
